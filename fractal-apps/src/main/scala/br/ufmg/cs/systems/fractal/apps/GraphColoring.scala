...
import br.ufmg.cs.systems.fractal.apps.Kcore.logInfo
import br.ufmg.cs.systems.fractal.util.Logging
import org.apache.spark.sql.{DataFrame, Row, SQLContext}
import org.apache.spark.sql.functions._
import org.apache.spark.{SparkConf, SparkContext}
import org.apache.spark.graphx.{Graph, VertexId, VertexRDD, Edge => GXEdge}
 ...
 */
object GraphColoring  extends Logging {
  ...
  */
  ...
  def colorGraphReductionFastest(g: GraphFrame, k: Int, maxIter:Int = 30): GraphFrame = {
    ...
    val gx1: Graph[VertexAttr, Row] = gx0.mapVertices { case (_, attr) =>
      val id = attr.getLong(vColsMap("id"))
      VertexAttr(id, id)
    }
  ...
  def colorGraphReductionNaive(g: GraphFrame, k: Int): GraphFrame = {
    ...
    val g2 = GraphFrame.fromGraphX(g, gxFinal, vertexNames = Seq("color"))
    val vertices = g2.vertices
    GraphFrame(vertices,g.edges)

  }

  def line2edge(string: String): Tuple2[Long, Long] = {
  ...
  def getColors(sc: SparkContext, path : String): List[Int] = {
    val startTimeMillis = System.currentTimeMillis()

    val lines = sc.textFile(path).map(line2edge)

    val sqlContext = SQLContext.getOrCreate(sc)

    val connections = sqlContext.createDataFrame(lines).toDF("srcIndex", "dstIndex")

    val edges = connections.withColumn("src", col("srcIndex")).withColumn("dst", col("dstIndex"))
      .select(col("src"), col("dst"))

    val g = GraphFrame.fromEdges(edges)
    val k = toInt(g.outDegrees.collect().maxBy(r => toInt(r.get(1))).get(1))

    //colorGraphReductionFastest ~27m
    val g2 = GraphColoring.colorGraphReductionNaive(g, k)
